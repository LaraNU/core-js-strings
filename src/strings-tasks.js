...
function formatTime(minutes, seconds) {
  ...
  const minPad = strMin.padStart(2, '0');
  const secPad = strSec.padStart(2, '0');
...
function countVowels(str) {
  ...
  let found = 0;
  if (str.length > 0 && str.match(regex) !== null) {
    found = str.match(regex).length;
  }
  return found;
}
...
function reverseWords(str) {
  ...
  const arr = [];
  array1.forEach((element) => arr.push(element.split('').reverse().join('')));
  return arr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < letters.length; i += 1) {
    if (letters[i].match(regex)) {
      if (letters[i].toUpperCase().charCodeAt() < 78) {
        letters[i] = String.fromCharCode(letters[i].charCodeAt() + 13);
      } else {
        letters[i] = String.fromCharCode(letters[i].charCodeAt() - 13);
      }
    }
...
function getCardId(value) {
  ...
  const suits = ['♣', '♦', '♥', '♠'];
  const cart = value.slice(0, -1);
  const suit = value.slice(-1);
  return tree.indexOf(cart) + suits.indexOf(suit) * 13;
}
